...
static void ReadFileInternal(const std::filesystem::path &filepath, std::ios_base::openmode mode, std::vector<BufferElemType>& outData) noexcept
{
    if (!std::filesystem::exists(filepath)) {
        ENG_LOG_WARN("File reading error. File {} doesn't exist.", filepath.string().c_str());
        return;
    }

    std::ifstream file(filepath, mode);
    if (!file.is_open()) {
        ENG_LOG_WARN("File reading error. Failed to open {} file.", filepath.string().c_str());
        return;
    }
...
{
    if (size == 0) {
        ENG_LOG_WARN("File {} writing warning. data size is 0", filepath.string().c_str());
        return;
    }

    if (data == nullptr) {
        ENG_LOG_WARN("File {} writing warning. data is nullptr", filepath.string().c_str());
        return;
    }

    std::ofstream file(filepath, mode);
    if (!file.is_open()) {
        ENG_LOG_WARN("File writing error. Failed to open {} file.", filepath.string().c_str());
        return;
    }
